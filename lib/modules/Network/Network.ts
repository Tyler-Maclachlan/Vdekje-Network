  ...
  constructor(
    ...
    for (let i = 0; i < this.data.nodes.length; i++) {
      ...
      if (typeof node.position != typeof Vector) {
        node.position = new Vector({x: node.position.x, y: node.position.y}, node.position.width, node.position.height);
      }
    ...
    this.physicsEngine = new PhysicsEngine(this.data.nodes, this.data.edges, { x: container.clientWidth / 2, y: container. clientHeight / 2});
